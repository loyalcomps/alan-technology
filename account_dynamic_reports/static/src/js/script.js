    ...
    var DynamicPaMain = AbstractAction.extend({
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                console.log("ssssssssssss",self.ageing_data)
                self.loader_enable_ui();
            });
        },
        ...
        fetch_move_lines: function(event) {
            event.preventDefault();
            console.log("FETCH MOVE LINESSSSSSSSSSSSSSSSSSSSSSSSS")
            var self = this;
            var partner_id = $(event.currentTarget).data('partner-id');
            var offset = 0;
            var td = $(event.currentTarget).next('tr').find('td');
        ...
        view_move_line: function(event) {
            event.preventDefault();
            console.log("view move lineeeeeeeeeeeeeeeeeeeeeee")
            var self = this;
            var context = {};
            var redirect_to_document = function(res_model, res_id, view_id) {
                var action = {
        ...
        update_with_filter: function(event) {
            ...
            for (var i = 0; i < partner_list.length; i++) {
                partner_ids.push(parseInt(partner_list[i].id))
                console.log("partnersssssssssssss",partner_ids)
            }
            output.partner_ids = partner_ids
            var partner_tag_ids = [];
    ...
    var DynamicTbMain = AbstractAction.extend({
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                self.filter_data.date_from = field_utils.format.date(field_utils.parse.date(self.filter_data.date_from, {}, {
                    isUTC: true
                }));
                self.filter_data.date_to = field_utils.format.date(field_utils.parse.date(self.filter_data.date_to, {}, {
                    isUTC: true
                }));
                if (initial_render) {
